...
\title{GRASP para a otimização do Índice de Qualidade Caminhável (IQC):\\ método clássico, implementação e análise dos resultados}
\author{}
\date{}
...
O projeto calcula, para cada hexágono H3 de uma região, um conjunto de indicadores de caminhabilidade e, a partir deles, o Índice de Qualidade Caminhável (IQC). A área de estudo é discretizada no conjunto de hexágonos $\mathcal{H} = \{h_1, h_2, \dots, h_{|\mathcal{H}|}\}$, e as dimensões formam o conjunto $\mathcal{K}$, com $|\mathcal{K}| = 11$: \code{S\_saude}, \code{S\_educacao}, \code{S\_abastecimento}, \code{S\_lazer}, \code{S\_servicos}, \code{I\_seguranca}, \code{A\_vegetacao}, \code{A\_agua}, \code{C\_conectividade}, \code{T\_transporte} e \code{U\_urbanidade}. O IQC de um hexágono é uma agregação ponderada dos indicadores, com pesos $w_j$ obtidos pelo método CRITIC.

A otimização consiste em alocar um orçamento fixo $B$ de pontos de interesse (POIs) na região, com o objetivo de maximizar a soma do IQC sobre todos os hexágonos. Os POIs só podem ser inseridos no subconjunto de dimensões instaláveis $\mathcal{K}_{POI} \subset \mathcal{K}$, com $|\mathcal{K}_{POI}| = 7$: \code{S\_saude}, \code{S\_educacao}, \code{S\_abastecimento}, \code{S\_lazer}, \code{S\_servicos}, \code{T\_transporte} e \code{U\_urbanidade}. Um POI inserido em um hexágono de origem não afeta apenas aquele hexágono: seu efeito se propaga para os hexágonos de destino segundo um modelo de impacto temporal. Cada par origem--destino tem um peso $\phi(t_{i',i})$ derivado do tempo de caminhada $t_{i',i}$ entre eles, por um decaimento em cosseno,
\begin{equation}
\phi(t) =
\begin{cases}
\dfrac{1 + \cos\!\left(\pi\,t/t_{max}\right)}{2}, & 0 \le t \le t_{max},\\[2mm]
0, & \text{caso contrário},
\end{cases}
\label{eq:alpha20}
\end{equation}
com $t_{max} = 20$ minutos ($\phi$ corresponde à coluna \code{alpha\_20} dos dados). Assim, inserir um POI perto de muitos destinos alcançáveis rende mais do que inseri-lo em um ponto isolado. O orçamento adotado é $B = 100$ POIs, e a soma do IQC sem nenhuma alocação é a linha de base (\emph{baseline}) usada para medir o ganho.

\subsection{Representação da solução e conjunto-base}

A correspondência com a teoria começa na definição do conjunto-base. Um elemento do conjunto-base $E$ é a inserção de uma unidade de POI em um par (hexágono de origem $h_i$, dimensão $j$). Uma solução é a matriz $\mathbf{\Delta X}$, de dimensão $|\mathcal{H}| \times |\mathcal{K}_{POI}|$, em que cada entrada $\Delta x_{i,j} \in \mathbb{Z}_{\ge 0}$ é a quantidade de POIs da dimensão $j$ alocada no hexágono $h_i$; as entradas somam o orçamento, $\sum_{i}\sum_{j} \Delta x_{i,j} = B$, e são não nulas apenas nas linhas correspondentes a hexágonos de origem elegíveis. Cada unidade alocada é, portanto, um elemento incorporado à solução.
...
$\mathcal{H}$, $|\mathcal{H}|$ & \code{h3\_ids}, \code{n\_hex} & conjunto de hexágonos e sua quantidade \\
$\mathcal{K}_{POI}$ & \code{candidate\_dimensions}, \code{n\_dims} & dimensões instaláveis de POI (sete) \\
$\mathcal{K}$ & \code{indicator\_columns} & os onze indicadores usados no IQC \\
...
$S_{i,j}$ & \code{baseline\_matrix} & oferta de base ($|\mathcal{H}| \times 11$) \\
$S^s_{i,j}(\mathbf{\Delta X})$ & \code{final\_matrix} & oferta após aplicar o impacto da alocação \\
$\Delta x_{i,j}$, $\mathbf{\Delta X}$ & \code{candidate\_matrix} (\code{cand}) & solução; quantidade de POIs no hexágono $h_i$, dimensão $j$ \\
$\phi(t_{i',i})$ & \code{alpha\_values} & peso de impacto do par origem $h_{i'}$ / destino $h_i$ \\
...
$\text{alcance}(i)$ & \code{reach\_row} & soma dos pesos de impacto que saem de $h_i$ \\
$w_j$ & \code{base\_weights} & peso CRITIC da dimensão $j$ \\
$\min_j,\max_j$ & \code{col\_min}, \code{col\_max} & mínimo e máximo da coluna $j$ na matriz corrente \\
--- & \code{range\_safe} & faixa $\max_j-\min_j$, com piso para evitar divisão por zero \\
--- & \code{col\_factor} & fator de coluna $w_j/(\max_j-\min_j)$ \\
...
\hat{S}^{s}_{i,j} =
\begin{cases}
\dfrac{S^{s}_{i,j} - \min_j}{\max_j - \min_j}, & \max_j > \min_j,\\[2mm]
0{,}5, & \max_j = \min_j,
\end{cases}
\end{equation}
e o IQC de um hexágono é a soma ponderada $\mathrm{IQC}_i = \sum_{j \in \mathcal{K}} w_j\, \hat{S}^{s}_{i,j}$, arredondada a quatro casas decimais. A aptidão de uma solução é o IQC global,
\begin{equation}
F(\mathbf{\Delta X}) = \overline{\mathrm{IQC}}(\mathbf{\Delta X}) = \sum_{i \in \mathcal{H}} \mathrm{IQC}_i,
\label{eq:objetivo}
\end{equation}
a ser maximizada; nas descrições dos algoritmos, $F(S)$ denota a aptidão da solução corrente $S = \mathbf{\Delta X}$. Os pesos $w_j$ vêm do CRITIC: para cada coluna calcula-se o desvio padrão da coluna normalizada, $\sigma_j$, e uma medida de conflito $\sum_{j'}\bigl(1 - |\rho_{j,j'}|\bigr)$, com $\rho_{j,j'}$ a correlação entre as colunas $j$ e $j'$; o produto $\sigma_j \sum_{j'}(1-|\rho_{j,j'}|)$ é normalizado para somar um.
...
\begin{equation}
\Delta F(i',j) \;\approx\; \frac{w_j}{\max_j - \min_j}\;\sum_{h_i \in \mathcal{N}_{i'}} \phi(t_{i',i})
\;=\; \frac{w_j}{\max_j - \min_j}\; \text{alcance}(i').
\label{eq:surrogate}
\end{equation}
A dedução vem de observar que aumentar o indicador $j$ em um destino $h_i$ por $\phi(t_{i',i})$ aumenta o IQC daquele destino em $w_j \cdot \phi(t_{i',i})/(\max_j - \min_j)$; somando sobre os destinos alcançáveis a partir de $h_{i'}$ chega-se à Equação~\eqref{eq:surrogate}. A estimativa é separável em um fator de linha, $\text{alcance}(i')$, e um fator de coluna, $w_j/(\max_j-\min_j)$, o que permite montar a grade inteira em uma operação de matriz, sem nenhuma avaliação da função objetivo. Os pesos $w_j$ ficam fixos, mas as faixas são recalculadas a cada passo (linhas~12 a~16); conforme uma dimensão é preenchida, sua faixa cresce e seu fator de coluna cai, o que produz retornos decrescentes e espalha as inserções entre as dimensões.
...
\begin{algorithm}[t]
\caption{GRASP implementado, por semente (maximização de $\overline{\mathrm{IQC}}$)}
...
A Figura~\ref{fig:grasp-fluxo} resume o fluxo de uma execução por semente, e a Figura~\ref{fig:grasp-anatomia} mostra como o orçamento de avaliações se distribui dentro de uma iteração. Os dois diagramas condensam o que as listagens anteriores detalham: a construção não consome avaliações; a avaliação da solução construída consome uma; e a busca local consome uma avaliação por vizinho testado, até o limite de $25 \times 64 = 1\,600$.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=3.6mm and 6mm]
\node[bloco] (seed) {semente $\rightarrow$ gerador pseudoaleatório};
\node[bloco,below=of seed] (pre) {pré-compila o escore surrogate\\ (pesos CRITIC da base, alcance por origem)};
\node[bloco,below=of pre] (alpha) {sorteia $\alpha \sim U[0,1]$};
\node[bloco,below=of alpha] (con) {construção gulosa randomizada\\ ($100$ inserções via RCL)};
\node[bloco,below=of con] (ev) {avalia $F(S)$};
\node[bloco,below=of ev] (ls) {busca local por primeira melhora};
\node[decisao,below=of ls] (dec) {$F(S) > F^\ast$?};
\node[bloco,below left=4.5mm and -14mm of dec] (upd) {$S^\ast \gets S$;\quad sem\_melhora $\gets 0$};
\node[bloco,below right=4.5mm and -14mm of dec] (noimp) {sem\_melhora $\gets$ sem\_melhora $+1$};
\node[decisao,below=17mm of dec] (stop) {parar?};
\node[bloco,below=of stop] (ret) {retorna $S^\ast$ e as métricas};
\node[rotulo,right=2.5mm of con] {$0$ avaliações};
...
\draw[seta] (stop.west) -- ++(-24mm,0) |- node[rotulo,pos=0.08,left]{não} (alpha.west);
\end{tikzpicture}
\caption{Fluxo de uma execução GRASP por semente, como implementado. Cada volta do laço é uma iteração independente: sorteio de $\alpha$, construção, avaliação, busca local e atualização da melhor solução.}
\label{fig:grasp-fluxo}
\end{figure}

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[x=1mm,y=1mm]
\draw[draw=black!55,fill=black!8]   (0,0)   rectangle (30,7);
\draw[draw=black!55,fill=black!16]  (30,0)  rectangle (38,7);
\draw[draw=black!55,fill=black!28]  (38,0)  rectangle (120,7);
\node[font=\small] at (15,3.5) {construção};
\node[font=\small] at (34,3.5) {aval.};
\node[font=\small] at (79,3.5) {busca local};
\draw[decorate,decoration={brace,mirror,amplitude=2mm}] (0,-1) -- (30,-1)
  node[rotulo,midway,below=2.5mm]{$0$ avaliações\\ ($\approx 8\%$ do tempo)};
\draw[decorate,decoration={brace,mirror,amplitude=2mm}] (30,-1) -- (38,-1)
  node[rotulo,midway,below=2.5mm]{$1$ avaliação};
\draw[decorate,decoration={brace,mirror,amplitude=2mm}] (38,-1) -- (120,-1)
  node[rotulo,midway,below=2.5mm]{$1$ a $1\,600$ avaliações\\ (avaliações $\approx 86\%$ do tempo total)};
\end{tikzpicture}
\caption{Anatomia de uma iteração GRASP sob a ótica do orçamento. A construção usa apenas o escore surrogate, sem chamar a função objetivo; o custo em avaliações concentra-se na busca local. As frações de tempo vêm dos registros de perfil das execuções.}
\label{fig:grasp-anatomia}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Matriz de oferta de base $S_{i,j}$ do exemplo.}
\label{tab:toy-base}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\label{sec:grasp-parametros}

Os valores da Tabela~\ref{tab:parametros} têm justificativas ligadas à teoria da Seção~\ref{sec:grasp-teoria}, ao custo computacional do problema e ao desenho da comparação com o ILS. Esta subseção percorre essas justificativas.
...
\label{sec:grasp-analise}

Esta seção analisa os resultados produzidos pela implementação do GRASP. Todos os números vêm dos arquivos gravados em \code{results/grasp/}, e todas as figuras foram geradas a partir desses arquivos.

\subsection{Desenho experimental}

Os experimentos cobrem cinco localidades (\code{av\_paulista}, \code{itajuba\_centro}, \code{metro\_ana\_rosa}, \code{milao\_italia}, \code{shinjuku\_toquio}) e três perfis de caminhada (\code{athlete}, \code{average\_adult}, \code{elderly}), totalizando quinze instâncias. Cada instância foi resolvida com as mesmas $100$ sementes de \code{seeds.txt} usadas pelo ILS, e cada semente é uma execução completa e independente, o que dá $1\,500$ execuções. Todas usaram a configuração padrão: teto de $30\,000$ avaliações, máximo de $500$ iterações, máximo de $80$ iterações sem melhora, $\alpha$ sorteado uniformemente em $[0,1]$ a cada iteração, e a busca local idêntica à do ILS ($64$ vizinhos por passo, $25$ passos).

\subsection{Metodologia estatística}

Para cada instância, as $100$ sementes são tratadas como uma amostra de $100$ observações independentes do valor final. Reporta-se média, desvio padrão amostral, mediana, intervalo interquartílico, coeficiente de variação e intervalo de confiança de $95\%$ para a média por $\bar{x} \pm t_{0{,}975;\,99}\, s/\sqrt{n}$. A melhoria sobre a linha de base é medida em porcentagem. A normalidade da distribuição dos valores finais entre sementes foi verificada pelo teste de Shapiro--Wilk: a hipótese de normalidade foi rejeitada ao nível de $5\%$ em sete das quinze instâncias, sempre com assimetria positiva; por isso as medianas são reportadas junto com as médias, e os testes usados nas demais análises são não paramétricos.

Três análises específicas do método usam testes próprios. Para verificar se o parâmetro $\alpha$ influencia o resultado, a distribuição do $\alpha$ da melhor iteração de cada execução é comparada com a distribuição uniforme por um teste de Kolmogorov--Smirnov: se $\alpha$ fosse irrelevante, o $\alpha$ da melhor iteração seria uniforme. Para medir a associação entre $\alpha$ e a qualidade das soluções, calcula-se a correlação de postos de Spearman entre o $\alpha$ da iteração e o valor após a busca local, separadamente dentro de cada execução (as iterações de uma mesma execução não são independentes entre execuções distintas, então a execução é a unidade de análise); reporta-se a média das $100$ correlações por instância, com intervalo de confiança, e um teste de Wilcoxon da hipótese de correlação nula. Para a forma da distribuição do IQC por hexágono, a assimetria e o excesso de curtose amostrais ($g_1$ e $g_2$, com correção de viés) são acompanhados dos testes de D'Agostino para assimetria e curtose.

\subsection{Resultados descritivos}

A Tabela~\ref{tab:grasp-desc} apresenta as estatísticas por instância e a Figura~\ref{fig:grasp-box} mostra a distribuição da melhoria porcentual sobre a base, por semente. Em todas as quinze instâncias, todas as $100$ sementes terminaram acima da linha de base. A dispersão entre sementes é baixa: o coeficiente de variação fica entre $0{,}19\%$ e $2{,}11\%$. O padrão de dependência da instância repete o já observado no projeto: a melhoria é grande onde a linha de base é baixa (Itajubá, entre $43$ e $51\%$) e pequena onde a base já é alta (Milão, entre $3{,}3$ e $6{,}1\%$).

\renewcommand{\arraystretch}{1.15}
\begin{longtable}{@{}l l c c c c c c c@{}}
\caption{Estatísticas descritivas do GRASP por instância ($100$ sementes). Valores em soma de IQC; $\Delta$IQC\% é a melhoria média sobre a base; CV\% é o coeficiente de variação; tempo em segundos.}\label{tab:grasp-desc}\\
\toprule
Localidade & Perfil & Base & Média & Desvio & Mediana & $\Delta$IQC\% & CV\% & Tempo méd. \\
\midrule
\endfirsthead
\toprule
Localidade & Perfil & Base & Média & Desvio & Mediana & $\Delta$IQC\% & CV\% & Tempo méd. \\
\midrule
\endhead
av\_paulista & athlete & 54,00 & 60,21 & 0,374 & 60,17 & 11,50 & 0,62 & 26,3 \\
av\_paulista & average\_adult & 47,06 & 53,43 & 0,461 & 53,41 & 13,55 & 0,86 & 18,3 \\
av\_paulista & elderly & 38,68 & 44,87 & 0,419 & 44,79 & 16,01 & 0,93 & 13,8 \\
itajuba\_centro & athlete & 30,23 & 43,27 & 0,710 & 43,26 & 43,12 & 1,64 & 19,9 \\
itajuba\_centro & average\_adult & 25,41 & 38,40 & 0,811 & 38,43 & 51,12 & 2,11 & 18,6 \\
itajuba\_centro & elderly & 22,75 & 32,99 & 0,691 & 32,93 & 45,00 & 2,10 & 13,7 \\
metro\_ana\_rosa & athlete & 42,54 & 53,15 & 0,604 & 53,11 & 24,93 & 1,14 & 32,3 \\
metro\_ana\_rosa & average\_adult & 38,53 & 47,77 & 0,573 & 47,77 & 23,97 & 1,20 & 20,5 \\
metro\_ana\_rosa & elderly & 32,45 & 37,06 & 0,378 & 37,07 & 14,23 & 1,02 & 14,4 \\
milao\_italia & athlete & 62,95 & 64,99 & 0,126 & 65,00 & 3,25 & 0,19 & 31,7 \\
milao\_italia & average\_adult & 57,45 & 59,90 & 0,267 & 59,84 & 4,27 & 0,45 & 25,4 \\
milao\_italia & elderly & 49,55 & 52,60 & 0,138 & 52,60 & 6,15 & 0,26 & 16,3 \\
shinjuku\_toquio & athlete & 57,10 & 60,25 & 0,189 & 60,22 & 5,52 & 0,31 & 28,5 \\
shinjuku\_toquio & average\_adult & 51,27 & 55,10 & 0,305 & 55,06 & 7,48 & 0,55 & 23,6 \\
shinjuku\_toquio & elderly & 42,23 & 46,74 & 0,357 & 46,70 & 10,68 & 0,76 & 18,3 \\
\bottomrule
\end{longtable}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/grasp_box_delta.pdf}
\caption{Distribuição, entre as $100$ sementes, da melhoria porcentual do GRASP sobre a linha de base, por instância. Caixas indicam quartis, a linha central é a mediana, e os pontos isolados são valores fora de $1{,}5$ vez o intervalo interquartílico.}
\label{fig:grasp-box}
\end{figure}

\subsection{Comportamento do parâmetro \texorpdfstring{$\alpha$}{alfa}}

Esta é a análise mais informativa sobre o método, porque o $\alpha$ é o único parâmetro genuíno do GRASP clássico. Duas perguntas orientam a análise: o $\alpha$ influencia o resultado? E, se influencia, existe um valor bom universal?

Para a primeira pergunta, considere o $\alpha$ da iteração que produziu a melhor solução de cada execução. Se o $\alpha$ fosse irrelevante, esse valor seria uniforme em $[0,1]$, já que os $\alpha$ são sorteados de maneira uniforme. O teste de Kolmogorov--Smirnov rejeita a uniformidade com folga ($D = 0{,}152$, $p \approx 1{,}2 \times 10^{-30}$, $n = 1\,500$): o painel esquerdo da Figura~\ref{fig:grasp-alpha} mostra a concentração em torno do centro do intervalo e o esvaziamento dos extremos. O $\alpha$ importa.

Para a segunda pergunta, o painel direito da Figura~\ref{fig:grasp-alpha} mostra a média, por instância, do $\alpha$ da melhor iteração. O intervalo é amplo: vai de $0{,}15$ em Milão (adulto e idoso) a $0{,}87$ em Itajubá (atleta). Ou seja, não existe um valor bom universal: em algumas instâncias as melhores soluções vêm de construções quase gulosas, em outras vêm de construções quase aleatórias. Esse resultado confirma nos dados a recomendação teórica da Seção~\ref{sec:grasp-teoria}, de que um único $\alpha$ fixo com frequência atrapalha, e sustenta a escolha do sorteio uniforme feita nesta implementação: o sorteio permite que cada instância encontre a sua faixa boa de $\alpha$ sem nenhum ajuste prévio.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/grasp_alpha.pdf}
\caption{À esquerda, distribuição agregada do $\alpha$ da melhor iteração nas $1\,500$ execuções, com a referência uniforme tracejada; a uniformidade é rejeitada por Kolmogorov--Smirnov ($p \approx 10^{-30}$). À direita, média por instância com intervalo de confiança de $95\%$: o melhor $\alpha$ varia de $0{,}15$ a $0{,}87$ conforme a instância.}
\label{fig:grasp-alpha}
\end{figure}

A Tabela~\ref{tab:grasp-alpha} quantifica a associação entre $\alpha$ e qualidade dentro das execuções, pela correlação de Spearman média entre o $\alpha$ da iteração e o valor da solução após a busca local. Todas as correlações médias são diferentes de zero (Wilcoxon, $p < 10^{-15}$ em todas as instâncias), mas o sinal muda: a correlação é positiva e forte em Itajubá (até $+0{,}95$), moderada na maior parte das instâncias, e negativa em Milão (até $-0{,}48$ no perfil idoso). A Figura~\ref{fig:grasp-alpha-scatter} mostra os dois padrões extremos: em Itajubá, quanto mais aleatória a construção, melhor a solução; em Milão, a curva sobe rápido até $\alpha$ por volta de $0{,}15$ e decai a partir daí, favorecendo construções quase gulosas.

O padrão tem uma explicação ligada à estrutura do problema. O escore surrogate favorece persistentemente as origens de maior alcance; com $\alpha$ pequeno, as inserções se concentram nessas origens. Em instâncias com poucas origens de alcance alto e base baixa, como Itajubá, essa concentração eleva demais o máximo das colunas e, pela normalização min--max do IQC, rebaixa os valores normalizados dos demais hexágonos — o mesmo mecanismo visto no exemplo da Seção~\ref{sec:exemplo}. Nessas instâncias, construções mais aleatórias espalham os POIs e produzem valores reais melhores. Em instâncias grandes e de base alta, como Milão, o efeito de concentração é diluído e a informação do escore guloso volta a valer.

\renewcommand{\arraystretch}{1.15}
\begin{longtable}{@{}l l c c c c@{}}
\caption{Análise do $\alpha$ por instância: média e mediana do $\alpha$ da melhor iteração, e correlação de Spearman média (entre iterações de uma execução, média sobre as $100$ execuções, com IC de $95\%$) entre o $\alpha$ e o valor após a busca local. Todas as correlações diferem de zero (Wilcoxon, $p<10^{-15}$).}\label{tab:grasp-alpha}\\
\toprule
Localidade & Perfil & $\bar{\alpha}$ melhor & Mediana & $\bar{\rho}_s(\alpha, F_{\text{LS}})$ & IC $95\%$ \\
\midrule
\endfirsthead
\toprule
Localidade & Perfil & $\bar{\alpha}$ melhor & Mediana & $\bar{\rho}_s(\alpha, F_{\text{LS}})$ & IC $95\%$ \\
\midrule
\endhead
av\_paulista & athlete & 0,440 & 0,354 & $+0{,}290$ & $[+0{,}267;\ +0{,}313]$ \\
av\_paulista & average\_adult & 0,414 & 0,399 & $+0{,}423$ & $[+0{,}403;\ +0{,}443]$ \\
av\_paulista & elderly & 0,545 & 0,524 & $+0{,}606$ & $[+0{,}590;\ +0{,}623]$ \\
itajuba\_centro & athlete & 0,871 & 0,880 & $+0{,}950$ & $[+0{,}946;\ +0{,}953]$ \\
itajuba\_centro & average\_adult & 0,834 & 0,827 & $+0{,}915$ & $[+0{,}910;\ +0{,}919]$ \\
itajuba\_centro & elderly & 0,751 & 0,753 & $+0{,}805$ & $[+0{,}797;\ +0{,}814]$ \\
metro\_ana\_rosa & athlete & 0,645 & 0,659 & $+0{,}593$ & $[+0{,}580;\ +0{,}607]$ \\
metro\_ana\_rosa & average\_adult & 0,569 & 0,565 & $+0{,}530$ & $[+0{,}513;\ +0{,}547]$ \\
metro\_ana\_rosa & elderly & 0,473 & 0,461 & $+0{,}634$ & $[+0{,}620;\ +0{,}648]$ \\
milao\_italia & athlete & 0,404 & 0,431 & $+0{,}078$ & $[+0{,}059;\ +0{,}097]$ \\
milao\_italia & average\_adult & 0,160 & 0,121 & $-0{,}277$ & $[-0{,}297;\ -0{,}258]$ \\
milao\_italia & elderly & 0,154 & 0,158 & $-0{,}485$ & $[-0{,}504;\ -0{,}466]$ \\
shinjuku\_toquio & athlete & 0,501 & 0,498 & $+0{,}416$ & $[+0{,}397;\ +0{,}436]$ \\
shinjuku\_toquio & average\_adult & 0,418 & 0,405 & $+0{,}217$ & $[+0{,}198;\ +0{,}235]$ \\
shinjuku\_toquio & elderly & 0,484 & 0,483 & $+0{,}324$ & $[+0{,}302;\ +0{,}347]$ \\
\bottomrule
\end{longtable}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/grasp_alpha_scatter.pdf}
\caption{Valor após a busca local em função do $\alpha$ da iteração (pontos: iterações de $25$ sementes; linha: média por faixa de $\alpha$), nas duas instâncias de comportamento extremo. Em Itajubá a qualidade cresce com $\alpha$; em Milão há um pico perto de $\alpha = 0{,}15$ seguido de queda.}
\label{fig:grasp-alpha-scatter}
\end{figure}

\subsection{Construção versus busca local}

A Tabela~\ref{tab:grasp-constr} separa a contribuição das duas fases. A coluna da melhor construção é o maior valor de solução construída (antes da busca local) da execução, em média sobre as sementes; a diferença até o melhor valor final mede o que a busca local acrescenta além da melhor construção pura. A busca local responde por $21$ a $38\%$ do ganho total sobre a base, com o restante vindo da construção — ou seja, a construção gulosa randomizada com escore surrogate produz sozinha a maior parte do ganho, e a busca local refina de forma consistente. A Figura~\ref{fig:grasp-constr-ls} mostra o efeito no nível da iteração: todos os pontos ficam sobre ou acima da reta identidade, porque a busca local por primeira melhora nunca piora a solução recebida.

\renewcommand{\arraystretch}{1.15}
\begin{longtable}{@{}l l c c c c@{}}
\caption{Decomposição do ganho entre construção e busca local (médias sobre $100$ sementes, em soma de IQC). A última coluna é a fração do ganho total sobre a base devida à busca local.}\label{tab:grasp-constr}\\
\toprule
Localidade & Perfil & Melhor construção & Melhor final & Acréscimo da BL & BL no ganho (\%) \\
\midrule
\endfirsthead
\toprule
Localidade & Perfil & Melhor construção & Melhor final & Acréscimo da BL & BL no ganho (\%) \\
\midrule
\endhead
av\_paulista & athlete & 58,18 & 60,21 & 2,03 & 32,7 \\
av\_paulista & average\_adult & 51,53 & 53,43 & 1,90 & 29,8 \\
av\_paulista & elderly & 43,39 & 44,87 & 1,48 & 23,9 \\
itajuba\_centro & athlete & 39,41 & 43,27 & 3,86 & 29,6 \\
itajuba\_centro & average\_adult & 35,01 & 38,40 & 3,39 & 26,1 \\
itajuba\_centro & elderly & 30,52 & 32,99 & 2,47 & 24,1 \\
metro\_ana\_rosa & athlete & 49,94 & 53,15 & 3,21 & 30,2 \\
metro\_ana\_rosa & average\_adult & 45,03 & 47,77 & 2,74 & 29,6 \\
metro\_ana\_rosa & elderly & 35,77 & 37,06 & 1,30 & 28,1 \\
milao\_italia & athlete & 64,31 & 64,99 & 0,69 & 33,5 \\
milao\_italia & average\_adult & 59,11 & 59,90 & 0,79 & 32,3 \\
milao\_italia & elderly & 51,97 & 52,60 & 0,63 & 20,6 \\
shinjuku\_toquio & athlete & 59,04 & 60,25 & 1,21 & 38,4 \\
shinjuku\_toquio & average\_adult & 53,81 & 55,10 & 1,29 & 33,6 \\
shinjuku\_toquio & elderly & 45,47 & 46,74 & 1,27 & 28,2 \\
\bottomrule
\end{longtable}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{figures/grasp_constr_ls.pdf}
\caption{Valor da solução construída contra o valor após a busca local, por iteração ($30$ sementes de Ana Rosa / adulto). Nenhum ponto fica abaixo da reta identidade: a busca local por primeira melhora nunca piora a solução.}
\label{fig:grasp-constr-ls}
\end{figure}
...
Todas as $1\,500$ execuções pararam por estagnação, ao atingir $80$ iterações sem melhora da melhor solução; nenhuma esgotou o orçamento de avaliações nem o limite de iterações. As execuções fizeram em média entre $137$ e $155$ iterações e gastaram entre $7\,800$ e $11\,700$ avaliações, ou seja, entre $26$ e $39\%$ do teto de $30\,000$. Cada iteração custou em média de $55$ a $78$ avaliações (uma da solução construída, o restante da busca local). O tempo por semente ficou entre $14$ e $32$ segundos.
...
A Figura~\ref{fig:grasp-conv} e a Tabela~\ref{tab:grasp-conv} mostram a convergência nas duas instâncias representativas usadas ao longo do capítulo. O perfil é o típico de multi-start: ganho muito rápido nas primeiras iterações, quando quase toda construção melhora o recorde, seguido de saturação. Por volta de $1\,000$ avaliações já se alcança cerca de $85\%$ da melhoria total; em $5\,000$, cerca de $96$ a $97\%$; a partir de $10\,000$ o ganho residual é inferior a $1\%$.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/grasp_convergence.pdf}
\caption{Convergência do GRASP: média, sobre as $100$ sementes, do melhor valor em função do número de avaliações (linha) e faixa interquartílica (área sombreada), em escala logarítmica de avaliações.}
\label{fig:grasp-conv}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Convergência: média do melhor valor por número de avaliações, e fração da melhoria total (entre parênteses).}
\label{tab:grasp-conv}
...
\caption{Assimetria ($g_1$) e excesso de curtose ($g_2$) da distribuição do IQC por hexágono, antes e depois da melhor alocação do GRASP. O símbolo $^{\dagger}$ marca valores significativamente diferentes de zero pelo teste de D'Agostino ao nível de $5\%$.}\label{tab:grasp-shape}\\
\toprule
...
\midrule
\endhead
av\_paulista & athlete & $+0{,}45^{\dagger}$ & $+0{,}06$ & $-0{,}37$ & $-0{,}89^{\dagger}$ \\
av\_paulista & average\_adult & $+0{,}52^{\dagger}$ & $+0{,}10$ & $-0{,}24$ & $-1{,}06^{\dagger}$ \\
av\_paulista & elderly & $+0{,}58^{\dagger}$ & $+0{,}28$ & $-0{,}07$ & $-0{,}60^{\dagger}$ \\
itajuba\_centro & athlete & $+0{,}82^{\dagger}$ & $+0{,}44^{\dagger}$ & $-0{,}34$ & $-0{,}66^{\dagger}$ \\
itajuba\_centro & average\_adult & $+1{,}08^{\dagger}$ & $+0{,}50^{\dagger}$ & $+0{,}34$ & $-0{,}35$ \\
itajuba\_centro & elderly & $+1{,}37^{\dagger}$ & $+0{,}66^{\dagger}$ & $+1{,}48^{\dagger}$ & $-0{,}07$ \\
metro\_ana\_rosa & athlete & $+0{,}76^{\dagger}$ & $+0{,}25$ & $+0{,}33$ & $-0{,}20$ \\
metro\_ana\_rosa & average\_adult & $+0{,}92^{\dagger}$ & $+0{,}30$ & $+0{,}85$ & $-0{,}34$ \\
metro\_ana\_rosa & elderly & $+1{,}01^{\dagger}$ & $+0{,}79^{\dagger}$ & $+1{,}03^{\dagger}$ & $+0{,}14$ \\
milao\_italia & athlete & $-0{,}09$ & $-0{,}15$ & $-0{,}93^{\dagger}$ & $-0{,}92^{\dagger}$ \\
milao\_italia & average\_adult & $+0{,}01$ & $-0{,}13$ & $-0{,}84^{\dagger}$ & $-0{,}99^{\dagger}$ \\
milao\_italia & elderly & $+0{,}13$ & $-0{,}00$ & $-0{,}62^{\dagger}$ & $-0{,}82^{\dagger}$ \\
shinjuku\_toquio & athlete & $+0{,}63^{\dagger}$ & $+0{,}44^{\dagger}$ & $-0{,}08$ & $-0{,}21$ \\
shinjuku\_toquio & average\_adult & $+0{,}77^{\dagger}$ & $+0{,}49^{\dagger}$ & $+0{,}33$ & $-0{,}13$ \\
shinjuku\_toquio & elderly & $+0{,}88^{\dagger}$ & $+0{,}56^{\dagger}$ & $+0{,}51$ & $-0{,}25$ \\
\bottomrule
\end{longtable}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/grasp_iqc_dist.pdf}
\caption{Distribuição do IQC por hexágono antes (cinza) e depois (azul) da melhor alocação do GRASP, nas duas instâncias representativas, com $g_1$ e $g_2$ anotados. Em Itajubá a cauda baixa esvazia e a distribuição se espalha para a direita; em Milão, já simétrica, a mudança é pequena.}
\label{fig:grasp-iqc-dist}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.66\textwidth]{figures/grasp_skewkurt.pdf}
\caption{Migração das quinze instâncias no plano assimetria $\times$ excesso de curtose, da distribuição de base (círculos vazios) para a distribuição após a melhor alocação do GRASP (círculos cheios). O movimento predominante é para a esquerda e para baixo: distribuições mais simétricas e menos concentradas.}
\label{fig:grasp-skewkurt}
\end{figure}

\subsection{Síntese}

Os dados sustentam cinco conclusões. Primeiro, o GRASP melhora a linha de base em todas as instâncias e todas as sementes, com dispersão baixa entre sementes (coeficiente de variação máximo de $2{,}1\%$), o que indica robustez. Segundo, o parâmetro $\alpha$ importa e seu valor bom depende da instância — o $\alpha$ da melhor iteração vai de $0{,}15$ a $0{,}87$ conforme o caso, e a correlação entre $\alpha$ e qualidade muda até de sinal —, o que confirma nos dados a escolha do sorteio uniforme de $\alpha$ e a advertência da teoria contra valores fixos. Terceiro, a construção responde pela maior parte do ganho (de $62$ a $79\%$) e a busca local refina o restante, sem nunca piorar. Quarto, o método converge cedo e para por estagnação em todas as execuções, gastando cerca de um terço do orçamento de avaliações; isso é coerente com o caráter de amostragem repetitiva sem memória do template clássico. Quinto, além de aumentar a soma do IQC, a intervenção torna a distribuição do IQC entre hexágonos mais simétrica e menos concentrada — a assimetria positiva significativa presente em doze instâncias antes da intervenção cai em quatorze delas depois —, o que na prática significa uma melhora mais bem distribuída pela região, e não restrita a poucos hexágonos.
